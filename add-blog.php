...
$categoryQuery = "SELECT id, name FROM blog_categories ORDER BY name ASC";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($title) && $category_id > 0 && isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            $insert = "INSERT INTO blogs (title, image, category_id, content) 
                       VALUES ('$title', '$image_name', $category_id, '$content')";
